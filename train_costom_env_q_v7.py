...
def get_state(pre_obs, obs, get_passenger, stations, passenger_loc):
    ...
    state = (passenger_loc, destination_dir, get_passenger, obstacle_north, obstacle_south, obstacle_east, obstacle_west) #, rel_stations[0], rel_stations[1], rel_stations[2], rel_stations[3]
...
def train_q_table(env, episodes=10000, alpha=0.2, gamma=0.99, epsilon_start=1.0, epsilon_end=0.01, decay_rate=0.9996):
    ...
    for episode in range(episodes):
        ...
        state = get_state(None, obs, get_passenger, stations, None)
        ...
        while not done:
            ...
            if (not get_passenger) and reward == 10 and action == 4:
                get_passenger = True
            if get_passenger and action == 5 and reward == -10:
                get_passenger = False

            next_state = get_state(obs, next_obs, get_passenger, stations, passenger_loc)
            # print(next_state)
            # env.render_env()
        ...
        if (episode + 1) % 100 == 0:
            ...
            with open("q_table_Q4_size5_4.pkl", 'wb') as file:
                pickle.dump(q_table, file)
            print(f"Q-table saved to q_table_Q4_size5_4.pkl")

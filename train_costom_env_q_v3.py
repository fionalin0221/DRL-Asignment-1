...
def get_state(obs, stations, passenger_loc, destination_loc, get_passenger):
    ...
    state = (taxi_row, taxi_col, passenger_loc, destination_loc, get_passenger, obstacle_north, obstacle_south, obstacle_east, obstacle_west, stations[0], stations[1], stations[2], stations[3])
...
def train_q_table(env, episodes=10000, alpha=0.2, gamma=0.99, epsilon_start=1.0, epsilon_end=0.01, decay_rate=0.9996):
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % 100 == 0:
            ...
            with open("q_table_Q4_random_station.pkl", 'wb') as file:
                pickle.dump(q_table, file)
            print(f"Q-table saved to q_table_Q4_random_station.pkl")

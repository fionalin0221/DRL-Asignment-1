...
class CostomEnv:
    ...
    def _generate_grid(self):
        # available_positions = self.all_positions - self.obstacles
        
        self.stations = set()
        while len(self.stations) < 4:
            candidate = random.choice(list(self.all_positions))
            if all(np.linalg.norm(np.array(candidate) - np.array(s), ord=1) >= 2 for s in self.stations):
                self.stations.add(candidate)

        # available_positions = available_positions - self.stations
